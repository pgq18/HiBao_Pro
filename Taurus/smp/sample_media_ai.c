...
static pthread_t g_PauseThread = 1;
...
int state = 0; // 0: empty; 1: audio recording; 2: audio end; 3: image
int capture_signal = 0;
int capture_end_signal = 0;
...
static HI_VOID* GetVpssChnFrameHandClassifySave(HI_VOID* arg)
{
    int ret;
    HI_S32 s32MilliSec = 20000;
    VO_LAYER voLayer = 0;
    VO_CHN voChn = 0;
    int gestureNameNow = -2;
    int t = 0;
    ...
    if (snprintf_s(achDstFileName, sizeof(achDstFileName), sizeof(achDstFileName) - 1,
        "/mnt/image/%d.yuv", cnt) < 0) {
        HI_ASSERT(0);
    }
    s32Ret = SampleIveColorConvertInit(&g_stColorSpaceInfo, pchSrcFileName, achDstFileName, HAND_FRM_WIDTH, HAND_FRM_HEIGHT);
    // s32Ret = SampleIveColorConvertInit(&g_stColorSpaceInfo, pchSrcFileName, achDstFileName, 1920, 1080);
    ...
    while (HI_FALSE == g_bAiProcessStopSignal) {
        ...
        if (gestureNameNow == 4 || CHECK_SWITCH_F1()) { // yes
            ...
            s32Ret = FrmToOrigImg(&resizeFrm, &img);
        ...
        else if (gestureNameNow == 3 || CHECK_SWITCH_F2()) { // palm
            ...
            printf("-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- get33333333333333 : audio save -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-\n");
        }
    }
...
static HI_VOID* GetVpssChnFrameSave(HI_VOID* arg)
{
    int ret;
    HI_S32 s32Ret;
    VIDEO_FRAME_INFO_S frm;
    ...
    if (snprintf_s(achDstFileName, sizeof(achDstFileName), sizeof(achDstFileName) - 1,
        "/complete_%s.yuv", "frm_orig_img") < 0) {
        HI_ASSERT(0);
    }
...
static HI_VOID* GetPause(HI_VOID* arg)
{
...
static HI_VOID* GetVpssChnFrameSaveSignal(HI_VOID* arg)
{
    printf("---------------enter get frame thread!---------------\n");
    int ret;
    HI_S32 s32Ret;
    VIDEO_FRAME_INFO_S frm;
    // VIDEO_FRAME_INFO_S resizeFrm;
    ...
    while (HI_FALSE == g_bAiProcessStopSignal) {
        t = 0;
        while(1) {
            if (CHECK_SWITCH_F1() == 1 || g_bAiProcessStopSignal) {
                t = 1;
                break;
            }
            // if (CHECK_SWITCH_F2() == 1) {
            //     t = 2;
            //     break;
            // }
        }
        if (g_bAiProcessStopSignal) {
            break;
        }
        printf("\n========== %d ==========\n", t);
        if (1) {
            printf("================================= SWITCH F1 PRESSED =================================\n");
...
static HI_S32 HandImageCaptureThreadProcess(HI_VOID)
{
    HI_S32 s32Ret;
    if (snprintf_s(acThreadName, BUFFER_SIZE, BUFFER_SIZE - 1, "ImageProcess") < 0) {
        HI_ASSERT(0);
    }
    prctl(PR_SET_NAME, (unsigned long)acThreadName, 0, 0, 0);
    s32Ret = pthread_create(&g_aiProcessThread, NULL, GetVpssChnFrameHandClassifySave, NULL);

    return s32Ret;
}

static HI_S32 ImageCaptureThreadProcess(HI_VOID)
{
    HI_S32 s32Ret;
    if (snprintf_s(acThreadName, BUFFER_SIZE, BUFFER_SIZE - 1, "ImageProcess") < 0) {
        HI_ASSERT(0);
    ...
    prctl(PR_SET_NAME, (unsigned long)acThreadName, 0, 0, 0);
    s32Ret = pthread_create(&g_aiProcessThread, NULL, GetVpssChnFrameSave, NULL);

    return s32Ret;
}

static HI_S32 ImageCaptureSignalThreadProcess(HI_VOID)
{
    HI_S32 s32Ret;
    if (snprintf_s(acThreadName, BUFFER_SIZE, BUFFER_SIZE - 1, "ImageProcess") < 0) {
        HI_ASSERT(0);
    ...
    prctl(PR_SET_NAME, (unsigned long)acThreadName, 0, 0, 0);
    s32Ret = pthread_create(&g_aiProcessThread, NULL, GetVpssChnFrameSaveSignal, NULL);

    return s32Ret;
}

static HI_S32 PauseThreadProcess(HI_VOID)
{
    HI_S32 s32Ret;
    if (snprintf_s(acThreadName, BUFFER_SIZE, BUFFER_SIZE - 1, "PauseProcess") < 0) {
        HI_ASSERT(0);
    }
    prctl(PR_SET_NAME, (unsigned long)acThreadName, 0, 0, 0);
    s32Ret = pthread_create(&g_PauseThread, NULL, GetPause, NULL);
...
HI_S32 DEMO_1(HI_VOID)
{
     ...
     * Create work thread to run ai
     */
    s32Ret = HandImageCaptureThreadProcess();
    SAMPLE_CHECK_EXPR_RET(s32Ret != HI_SUCCESS, s32Ret, "ai proccess thread creat fail:%s\n", strerror(s32Ret));

    s32Ret = PauseThreadProcess();
    SAMPLE_CHECK_EXPR_RET(s32Ret != HI_SUCCESS, s32Ret, "pause proccess thread creat fail:%s\n", strerror(s32Ret));

    // Pause();
    // g_bAiProcessStopSignal = HI_TRUE;
    AUDIO_INIT();
    int t = 0;
    while(!g_bAiProcessStopSignal) {
        printf("\n======================== please press F2 to enter audio recording ========================\n");
        while(1) {
            // if (CHECK_SWITCH_F2() == 1 || g_bAiProcessStopSignal) {
            if (state == 1 || g_bAiProcessStopSignal) {
                t = 2;
                break;
            }
            if (capture_signal == 1) {
                capture_signal = 0;
                AUDIO_READ_START_TAKEING_PHTOT();
            }
            if (capture_end_signal == 1) {
                capture_end_signal = 0;
                AUDIO_READ_START_AFTER_PHTOT();
            }
        }
        if (g_bAiProcessStopSignal) {
            break;
        ...
        //     break;
        // }
        AUDIO_READ_START_START_RECORD();
        AUDIO_START();
        AUDIO_READ_START_END_RECORD();
        t = 0;
        // break;
    }
    AUDIO_STOP();
    ...
    SAMPLE_COMM_IVE_IveMpiExit();
    pthread_join(g_aiProcessThread, NULL);
    pthread_join(g_PauseThread, NULL);
    g_aiProcessThread = 0;
    g_PauseThread = 1;
    PauseDoUnloadHandClassifyModel();
 ...
 * VI->VPSS->VO->MIPI
 */
HI_S32 IMAGE_DISPLAY_INNER(HI_VOID)
{
    HI_S32             s32Ret;
    HI_S32             fd = 0;
...
/*
 * 将sensor采集到数据显示到液晶屏上，同时创建线程使用按键F1抓拍图片并保存在本地
 * 视频输入->视频处理子系统->视频输出->显示屏
 *
 * Display the data collected by the sensor on the LCD screen,
 * and create threads to capture images while button F1 is pressed, and save images.
 * VI->VPSS->VO->MIPI
 */
HI_S32 IMAGE_CAPTURE_INNER(HI_VOID)
{
    HI_S32             s32Ret;
    HI_S32             fd = 0;
    ...
    /*
     * 创建工作线程运行图片抓取
     * Create work thread to run image capturing
     */
    // SAMPLE_COMM_IVE_CheckIveMpiInit();
    s32Ret = ImageCaptureSignalThreadProcess();
    SAMPLE_CHECK_EXPR_RET(s32Ret != HI_SUCCESS, s32Ret, "image proccess thread creat fail:%s\n", strerror(s32Ret));
