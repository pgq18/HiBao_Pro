...
static FILE *SAMPLE_AUDIO_OpenAencFile(AENC_CHN AeChn, PAYLOAD_TYPE_E enType)
{
    ...
    s32Ret = snprintf_s(aszFileName, FILE_NAME_LEN, FILE_NAME_LEN - 1,
        "/mnt/audio_chn%d.%s", AeChn, SAMPLE_AUDIO_Pt2Str(enType));
    if (s32Ret <= EOK) {
        printf("\n snprintf_s fail! ret = 0x%x", s32Ret);
        return NULL;
    }
...
static FILE *SAMPLE_AUDIO_OpenAdecFile(ADEC_CHN AdChn, PAYLOAD_TYPE_E enType, int t)
{
    FILE *pfd = NULL;
    HI_CHAR aszFileName[FILE_NAME_LEN] = {0};
    HI_S32 s32Ret;
    ...
    switch(t) {
        case 0:
            s32Ret = snprintf_s(aszFileName, FILE_NAME_LEN, FILE_NAME_LEN - 1, "./acc_files/startrecord.aac");
            break;
        case 1:
            s32Ret = snprintf_s(aszFileName, FILE_NAME_LEN, FILE_NAME_LEN - 1, "./acc_files/endrecord.aac");
            break;
        case 2:
            s32Ret = snprintf_s(aszFileName, FILE_NAME_LEN, FILE_NAME_LEN - 1, "./acc_files/takephoto.aac");
            break;
        case 3:
            s32Ret = snprintf_s(aszFileName, FILE_NAME_LEN, FILE_NAME_LEN - 1, "./acc_files/afterphoto.aac");
            break;
        default:
            s32Ret = snprintf_s(aszFileName, FILE_NAME_LEN, FILE_NAME_LEN - 1, "./acc_files/takephoto.aac");
    }
    // s32Ret = snprintf_s(aszFileName, FILE_NAME_LEN, FILE_NAME_LEN - 1, "./acc_files/startrecord.acc");
    // s32Ret = snprintf_s(aszFileName, FILE_NAME_LEN, FILE_NAME_LEN - 1,
    //     "/mnt/audio_chn%d.%s", AdChn, SAMPLE_AUDIO_Pt2Str(enType));
    if (s32Ret <= EOK) {
        printf("\n snprintf_s fail! ret = 0x%x", s32Ret);
        return NULL;
    }
...
static HI_VOID SAMPLE_AUDIO_AiAencInner(AUDIO_DEV AiDev, AUDIO_DEV AoDev, HI_S32 s32AencChnCnt, AIO_ATTR_S *pstAioAttr)
{
    ...
    while(1) {
        printf("\n======================== please press F1 to enter this sample ========================\n");
        while(1) {
...
AENC_CHN AeChn_ = 0;
AI_CHN AiChn_;
const ADEC_CHN AdChn_ = 0;
AIO_ATTR_S stAioAttr_;
...
//     }
    ...
    sample_audio_ai_aenc_init_param(&stAioAttr_);

    /* step 1: start Ai */
    s32AiChnCnt_ = stAioAttr_.u32ChnCnt;
    s32Ret = SAMPLE_COMM_AUDIO_StartAi(AiDev, s32AiChnCnt_, &stAioAttr_, g_out_sample_rate, gs_bAioReSample, NULL, 0);
    if (s32Ret != HI_SUCCESS) {
        SAMPLE_DBG(s32Ret);
        return s32Ret;
    }
    ...
    if (s32Ret != HI_SUCCESS) {
        SAMPLE_DBG(s32Ret);
        s32Ret = SAMPLE_COMM_AUDIO_StopAi(AiDev, s32AiChnCnt_, gs_bAioReSample, HI_FALSE);
        if (s32Ret != HI_SUCCESS) {
            SAMPLE_DBG(s32Ret);
        }
        return s32Ret;
    ...
    /* step 3: start Aenc */
    s32AencChnCnt_ = stAioAttr_.u32ChnCnt >> (HI_U32)stAioAttr_.enSoundmode;
    s32Ret = SAMPLE_COMM_AUDIO_StartAenc(s32AencChnCnt_, &stAioAttr_, gs_enPayloadType);
    if (s32Ret != HI_SUCCESS) {
        SAMPLE_DBG(s32Ret);
        s32Ret = SAMPLE_COMM_AUDIO_StopAi(AiDev, s32AiChnCnt_, gs_bAioReSample, HI_FALSE);
        if (s32Ret != HI_SUCCESS) {
    ...
    for (i = 0; i < s32AencChnCnt_; i++) {
        AeChn_ = i;
        AiChn_ = i;

        if (gs_bUserGetMode == HI_TRUE) {
            s32Ret = SAMPLE_COMM_AUDIO_CreateTrdAiAenc(AiDev, AiChn_, AeChn_);
        } else {
            s32Ret = SAMPLE_COMM_AUDIO_AencBindAi(AiDev, AiChn_, AeChn_);
        }
        if (s32Ret != HI_SUCCESS) {
            SAMPLE_DBG(s32Ret);
            for (j = 0; j < i; j++) {
            ...
            return s32Ret;
            printf("Ai(%d,%d) bind to AencChn:%d fail!\n", AiDev, AiChn_, AeChn_);
        }
        printf("Ai(%d,%d) bind to AencChn:%d ok!\n", AiDev, AiChn_, AeChn_);
    }
...
HI_S32 AUDIO_READ_START_AFTER_PHTOT(HI_VOID)
{
    ...
    pfd = SAMPLE_AUDIO_OpenAdecFile(AdChn_, gs_enPayloadType, 3);
    if (pfd == NULL) {
        SAMPLE_DBG(HI_FAILURE);
...
HI_S32 AUDIO_READ_START_TAKEING_PHTOT(HI_VOID)
{
    ...
    pfd = SAMPLE_AUDIO_OpenAdecFile(AdChn_, gs_enPayloadType, 2);
    if (pfd == NULL) {
        SAMPLE_DBG(HI_FAILURE);
        s32Ret = SAMPLE_COMM_AUDIO_AoUnbindAdec(AoDev, AoChn, AdChn_);
        if (s32Ret != HI_SUCCESS) {
...
HI_S32 AUDIO_READ_START_START_RECORD(HI_VOID)
{
    ...
    pfd = SAMPLE_AUDIO_OpenAdecFile(AdChn_, gs_enPayloadType, 0);
    if (pfd == NULL) {
        SAMPLE_DBG(HI_FAILURE);
        s32Ret = SAMPLE_COMM_AUDIO_AoUnbindAdec(AoDev, AoChn, AdChn_);
        if (s32Ret != HI_SUCCESS) {
...
HI_S32 AUDIO_START(HI_VOID)
{
    ...
    // int t = 0;
    // while(1) {
    //     printf("\n======================== please press F1 to enter audio recording ========================\n");
    //     while(1) {
    //         // if (CHECK_SWITCH_F1() == 1) {
    //         //     t = 1;
    //         //     break;
...
HI_S32 AUDIO_STOP(HI_VOID)
{
    ...
    for (i = 0; i < s32AencChnCnt_; i++) {
        ...
        if (gs_bUserGetMode == HI_TRUE) {
            s32Ret = SAMPLE_COMM_AUDIO_DestroyTrdAi(AiDev, AiChn_);
        } else {
            s32Ret = SAMPLE_COMM_AUDIO_AencUnbindAi(AiDev, AiChn_, AeChn_);
        }
        if (s32Ret != HI_SUCCESS) {
            SAMPLE_DBG(s32Ret);
        }
